...
static int admission_dfs(Service *service, int environment_id, int64_t ts,
  ...
  for (size_t i = 0; i < service->dependency_count; ++i) {
    Service *dep = find_service(service->dependencies[i]);
    if (dep == NULL || !admission_dfs(dep, environment_id, ts, state)) {
      state[idx] = 3U;
      return 0;
    }
  }
...
static int check_admission_known(Service *service, int environment_id, int64_t ts,
                                 unsigned char *shared_state) {
  if (shared_state != NULL) {
    return admission_dfs(service, environment_id, ts, shared_state);
  }

  unsigned char *state = (unsigned char *)calloc(g_service_count, sizeof(*state));
  if (state == NULL) {
    g_last_error = GATE_ERR_NO_MEMORY;
    return 0;
  }

  int result = admission_dfs(service, environment_id, ts, state);
  free(state);
  return result;
}
...
__attribute__((visibility("default"))) int gate_check_admission(int service_id,
  ...
  int result = check_admission_known(service, environment_id, ts, NULL);
...
__attribute__((visibility("default"))) int gate_audit_get(int service_id,
  ...
  EnvState *env = find_env(service, environment_id);
  EvidenceView evidence = evaluate_evidence(service, environment_id, ts);
  unsigned char *visiting = (unsigned char *)calloc(g_service_count, sizeof(*visiting));
  if (visiting == NULL) {
    g_last_error = GATE_ERR_NO_MEMORY;
    return 0;
  }
  ...
  out_view->admissible = check_admission_known(service, environment_id, ts, NULL);
  if (g_last_error == GATE_ERR_NO_MEMORY) {
    return 0;
  }
...
__attribute__((visibility("default"))) int gate_count_admissible(int environment_id,
  ...
  unsigned char *state = (unsigned char *)calloc(g_service_count, sizeof(*state));
  if (state == NULL && g_service_count != 0U) {
    g_last_error = GATE_ERR_NO_MEMORY;
    return 0;
  }
  ...
  int count = 0;
  for (size_t i = 0; i < g_service_count; ++i) {
    if (check_admission_known(&g_services[i], environment_id, ts, state)) {
      if (count < INT_MAX) {
        ++count;
      }
    }
