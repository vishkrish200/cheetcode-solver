...
describe("resolveGithubIdentity", () => {
  it("requires an explicit, valid GitHub handle", () => {
    expect(() => resolveGithubIdentity(undefined)).toThrow(/CHEETCODE_GITHUB/);
    expect(resolveGithubIdentity(" example-user ")).toBe("example-user");
    expect(() => resolveGithubIdentity("not a handle")).toThrow(/Invalid/);
  });
});
...
describe("current session replay lifecycle", () => {
  ...
  it("matches the current client summary shape", () => {
    const summary = buildReplaySummary(session, "example-user", buildFingerprintHints("fp-1", 1234), solved);
