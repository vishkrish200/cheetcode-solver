/**
 * Run a no-network rehearsal against previously saved challenge artifacts.
 *
...
const OUTPUT_ROOT = path.resolve("recon-output");
const DEFAULT_CATALOG_DIR = path.join(OUTPUT_ROOT, "2026-05-21T18-website-clue-hunt", "all-advertised-chunks");
...
async function main(): Promise<void> {
  ...
  const sourceRunDir = path.resolve(options.runDir ?? (await findLatestCompleteRunDir()));
...
async function findLatestCompleteRunDir(): Promise<string> {
  ...
  if (!latest) {
    throw new Error(`No complete saved run found under ${OUTPUT_ROOT}. Pass --run-dir explicitly.`);
  }
  return latest;
}
...
This is strictly offline. It reads saved session artifacts, solves Level 1 locally,
matches Level 2 against the saved catalog, and compiles a verified Level 3 candidate.
